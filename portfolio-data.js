...
In D:\Ascentia (read ops\ARTIFACT_HYGIENE.md first): delete ONLY the approved cache-class paths. Laws: repos\landscry\unreal\Binaries is generated-state, NOT cache — if I listed it, first prove a clean rebuild of the winding-fixed importer DLLs; never touch evidence/artifact/generated-state classes under a cache approval; before deleting a tree's caches confirm no editor or build is running against it (any open UE 5.8 editor blocks builds repo-wide via the Live Coding mutex); UE recreates DDC/Intermediate on demand so the directories can go entirely. Afterwards run the full refresh (refresh-portfolio.ps1) and report the reclaimed GB from the Hygiene tiles, before/after. Close out with the Portfolio Signal (update this hand-off or prune it).`,
    ...
    {
      id: 'landscry-conf01-harness',
      title: 'CONF-01 conformance harness: automate the control ladder — plus your one credential step',
      target: 'Any agent · D:\Ascentia\repos\landscry',
      decision: false,
      why: 'CTRL-02 shipped 2026-08-04: erosion_intervals (1-100) is the second public control, measured strictly monotone on its declared metric (local_relief_p90 0.938 to 0.720, warn to pass) with 1.3% cross-control drift - and running the ladder caught a real defect: every published metric had been computed on the pre-sediment bed, a surface bundles never ship. All published metrics now measure the published presentation surface. The goal ordering says the conformance harness is next: automate exactly that ladder (monotonicity + cross-control tolerance) for every registered control, retrofitting CTRL-01 and CTRL-02, so the check that caught this defect runs on every future control by construction. The frontier composition credential step remains yours - one step, no code.',
      prompt: String.raw`Ship CONF-01: the control conformance harness (goal scope item 2).
...
[NO USER DECISION REQUIRED for the slice itself. Do not reopen Unreal; frozen fixture untouched; defaults stay byte-identical no-ops.]

Start in D:\Ascentia\repos\landscry on branch agent/s1-sediment (keep PR #2 open for CI). Read FIRST: .agent\GOAL.md (scope item 2 + acceptance criterion 3), docs\CTRL02_EROSION_DEPTH_2026-08-04.md (the manual ladder this slice automates, and the wrong-surface defect it caught), ADR-0026 + ADR-0031, and the Landscry lane row in D:\Ascentia\ops\PIPELINE_STATUS.md.

THE SLICE: an automated test (or cargo-run harness with a committed record) that, for EVERY control in the m3 registry, builds its boundary ladder through the product path and asserts (1) the declared metric moves monotonically in the declared direction, and (2) no OTHER registered control's declared metric moves beyond a recorded tolerance (the relief-vs-water measurement of 2026-07-28 and the 1.3% gross_relief drift in CTRL02 evidence are the templates). Retrofit CTRL-01 (relief_scale to gross_relief_m, up) and CTRL-02 (erosion_intervals to local_relief_p90_m from the published scorecard, down) as the first two subjects. Use small rungs (512 body) so the harness is CI-viable; the measured 2048 ladders in the evidence docs remain the deep record.

EXIT EVIDENCE: (1) harness red if a control's declared metric goes flat (regression-test the exact CTRL-02 wrong-surface failure mode: a metric computed off a surface the bundle does not publish must be caught); (2) monotonicity + tolerance table recorded for both controls; (3) registry entries without a live measured axis fail the harness visibly.

Laws: green-gated bites; fmt after every edit; frozen fixture untouched; no gate promotion; honest claims.
...
In D:\Ascentia\repos\landscry (read AGENTS.md and the mountain-region lane row in D:\Ascentia\ops\PIPELINE_STATUS.md first): the validated 16-tile UE window package t4b-native-de1c19affdbe624b sits in artifacts\mountain-region-20260718 (225 MB, 80 artifacts, all checks green, same source identity as the review bundle). Run the import: one source round of 16 tiles + the WP MeshPartition builder with ByPackageHash reuse (verify-don't-resave definition assets) + optional HLOD pass as ONE BUILDER PROCESS PER HLOD ACTOR via -BuildSingleHLOD (labels carry the <HLODLayer>/ prefix) + inspect with the winding gate. Gotchas: editing anything under crates/ + Cargo + fixtures mid-build forks the engine cache identity; close editors before headless builds (Live Coding patch-DLL law; bypass flag is -NoHotReloadFromIDE); the terrain/water MATERIAL is still the open T4b presentation gate — this import proves geometry and streaming, not final look. Deliver a lit, flyable level plus its path for me to fly myself — renders are internal verification only. Close out with the Portfolio Signal.`,
  ...
  projects: [
    ...
    },
    {
      ...
      next: 'Two tracks: YOUR one-step credential (ANTHROPIC_API_KEY or ant auth login, then Compose - first live composition = the MACRO-02 acceptance run) and CONF-01, the conformance harness that automates the control ladder which just caught the wrong-surface defect. Shipped today: MACRO-03 (macro bands measured in-product), composer round 1 (five worlds, four defects named), CTRL-02 (erosion depth, monotone warn-to-pass on its declared metric). Queued: MACRO-04 families, amplitude fidelity, SED-07.',
