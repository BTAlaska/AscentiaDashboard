...
window.PORTFOLIO_DATA = {
  ...
  priorities: [
    ...
    {
      level: 'active',
      title: 'Landscry: new long-running goal — DEM-grade text-to-world',
      body: 'Owner authorized CTRL-02+ on 2026-07-28 and asked for a goal that massively improves what a prompt actually produces. The goal is recorded in .agent/GOAL.md and succeeds only if the T3 real-DEM ledger moves off its 15 fail / 2 warn baseline — not on a count of shipped controls. Three findings make it tractable rather than speculative. The product physically cannot measure itself today: the calibration crate is a dev-dependency of the engine, so a published bundle carries one hand-rolled metric while drainage density, mean slope, concavity and hypsometry all sit behind that wall. All five DEM-calibrated family profiles already exist in the engine with an 18-parameter config matching the corpus value for value, but four are unreachable from the product — one geology module against a corpus that defines five. And the shipped hero world deliberately diverges from calibrated alpine (6x sparser channels) and is then measured against families it was never tuned to match, which is a product tension for YOU to resolve rather than a bug to silently fix.',
      owner: 'Landscry lane / one owner decision flagged inside the goal',
    },
    {
      level: 'active',
...
In D:\Ascentia (read ops\ARTIFACT_HYGIENE.md first): delete ONLY the approved cache-class paths. Laws: repos\landscry\unreal\Binaries is generated-state, NOT cache — if I listed it, first prove a clean rebuild of the winding-fixed importer DLLs; never touch evidence/artifact/generated-state classes under a cache approval; before deleting a tree's caches confirm no editor or build is running against it (any open UE 5.8 editor blocks builds repo-wide via the Live Coding mutex); UE recreates DDC/Intermediate on demand so the directories can go entirely. Afterwards run the full refresh (refresh-portfolio.ps1) and report the reclaimed GB from the Hygiene tiles, before/after. Close out with the Portfolio Signal (update this hand-off or prune it).`,
    ...
    {
      id: 'landscry-dem-grade-text-to-world',
      title: 'Run the DEM-grade text-to-world goal: baseline, then the measurement spine',
      target: 'Any agent · D:\\Ascentia\\repos\\landscry',
      decision: false,
      why: 'DECIDED 2026-07-28: you chose option A and authorized CTRL-02+, and the long-running goal is now recorded in .agent/GOAL.md (adc94a8). The first two steps are mechanical and unblock everything after them. Right now the product physically cannot measure its own terrain against real DEMs — the calibration crate is a dev-dependency of the engine, so a published bundle carries exactly one hand-rolled metric. Until that changes, no fidelity claim in this goal is measurable.',
      prompt: String.raw`Run steps 1 and 2 of the DEM-grade text-to-world goal: re-derive the T3 baseline, then promote the calibration metric set into the product build path.

[NO USER DECISION REQUIRED. The owner authorized CTRL-02+ on 2026-07-28. Treat that as scoped to named control slices with recorded evidence, not as blanket permission to edit terrain arithmetic. Do not reopen Unreal.]

Start in D:\Ascentia\repos\landscry on branch agent/s1-sediment (PR #2 is open; CI runs only on main and pull requests, so keep the PR open or your work gets no CI). Read .agent\GOAL.md FIRST — it is the active goal and it records the findings below with file and line citations. Then AGENTS.md, docs\PRODUCT_CONSTITUTION.md, docs\adr\0018-dem-calibration-v1.md, docs\T3_PHASE_ATTRIBUTION_2026-07-15.md, and the Landscry lane row in D:\Ascentia\ops\PIPELINE_STATUS.md.

STEP 1 — re-derive the baseline. The often-quoted "15 fail / 2 warn" is an aggregate bound to target\t3-runs\..., and target\ is gitignored, so which windows fail and which metrics fail in them exist nowhere in the repository. Run the sealed assembler (crates\landscry-terrain-calibration\examples\t3_evidence_assemble.rs via scripts\run-t3-candidate-evidence.ps1) at the current head and record the per-window, per-family, per-metric result as a checked-in evidence document. This is the number every later claim in this goal is compared against. Budget it: the full corpus run is expensive. Do not estimate any figure you did not measure.

STEP 2 — promote the metric set into the product path. Today landscry-terrain-calibration is a DEV-dependency of landscry-engine, so crates\landscry-engine\src\t4b_review.rs can compute exactly one control metric, gross_relief_m, hand-rolled at line 797. drainage_density_km_per_km2, mean_slope_degrees, slope_area_concavity, local_relief_p90_m, channel_head_spacing_m, valley_floor_width_m, and normalized_hypsometry_512 all exist in the calibration crate and none reaches a published bundle. Resolve that boundary so the product can compute them in src\ WITHOUT duplicating the implementation — one metric implementation serving both the T3 harness and the published bundle. Then publish a per-family conformance scorecard in every review bundle, bind the corpus and metric versions into provenance, and extend landscry.acceptance_report from one axis to the declared set. A metric that cannot be computed reports unavailable, never a pass.

Laws that bind both steps: the reference world at default controls must stay byte-identical and every frozen characterization gate must stay green; promote no gate (this work improves the ledger, it does not pass CAL-G5/G6/G7); a worse verdict on any metric stops for owner review rather than being averaged away; and claims stay at the level their own gates have passed.

Context worth knowing before you plan: the recorded failure ranking is local_relief_p90_m 11 of 13 windows, mean_slope_degrees 11, slope_area_concavity 8, slope_histogram_l1 6, curvature_histogram_l1 4, and ZERO recorded failures for drainage density, channel head spacing, valley floor width, relief_m, aspect anisotropy, return flow, or hypsometry. Generated mean slope runs 2.1-3.9x reference and local relief 3.4-15.1x — the terrain is too steep and too tall, which is a magnitude problem, not a pattern problem.

After these two steps the queued work is the conformance harness (nothing currently asserts a control moves its declared metric in its declared direction) and then CTRL-02 = erosion time-depth, which is chosen because SED-06 re-routing already took alpine local_relief_p90_m from 1.302 FAIL to 0.850 PASS at 100 intervals. Do NOT use channel-initiation threshold: it was implemented and reverted because it does not drive water.channel_cells.

Validation: cargo fmt --all -- --check; cargo check --locked --workspace --all-targets; cargo clippy --locked --workspace --all-targets -- -D warnings; cargo test --locked --workspace; corepack pnpm check; git diff --check. Close out with the Portfolio Signal per D:\Ascentia\repos\dashboard\PORTFOLIO_UPDATE_PROTOCOL.md, update .agent\ITERATION_LOG.md, refresh the tracker, and sweep artifacts per D:\Ascentia\ops\ARTIFACT_HYGIENE.md.`,
    },
    {
...
In D:\Ascentia\repos\landscry (read AGENTS.md and the mountain-region lane row in D:\Ascentia\ops\PIPELINE_STATUS.md first): the validated 16-tile UE window package t4b-native-de1c19affdbe624b sits in artifacts\mountain-region-20260718 (225 MB, 80 artifacts, all checks green, same source identity as the review bundle). Run the import: one source round of 16 tiles + the WP MeshPartition builder with ByPackageHash reuse (verify-don't-resave definition assets) + optional HLOD pass as ONE BUILDER PROCESS PER HLOD ACTOR via -BuildSingleHLOD (labels carry the <HLODLayer>/ prefix) + inspect with the winding gate. Gotchas: editing anything under crates/ + Cargo + fixtures mid-build forks the engine cache identity; close editors before headless builds (Live Coding patch-DLL law; bypass flag is -NoHotReloadFromIDE); the terrain/water MATERIAL is still the open T4b presentation gate — this import proves geometry and streaming, not final look. Deliver a lit, flyable level plus its path for me to fly myself — renders are internal verification only. Close out with the Portfolio Signal.`,
  ...
  projects: [
    ...
    },
    {
      ...
      focus: 'New long-running goal as of 2026-07-28: DEM-grade text-to-world (.agent/GOAL.md). Make the product able to measure itself against the real-DEM corpus, expose enough calibrated controls and families for a prompt to steer, then close the fidelity gap axis by axis. Owner authorized CTRL-02+, so the core terrain DEM is unfrozen for named control slices with recorded evidence. Unreal stays shelved and all M6 commercial work stays out of scope.',
      ...
      next: 'Goal steps 1 and 2 are done. Next is the automated conformance harness — nothing currently asserts a control moves its declared metric in its declared direction — and then CTRL-02, erosion time-depth, chosen because it is the one mechanism with recorded evidence of moving the metrics that actually fail.',
