...
In D:\Ascentia (read ops\ARTIFACT_HYGIENE.md first): delete ONLY the approved cache-class paths. Laws: repos\landscry\unreal\Binaries is generated-state, NOT cache — if I listed it, first prove a clean rebuild of the winding-fixed importer DLLs; never touch evidence/artifact/generated-state classes under a cache approval; before deleting a tree's caches confirm no editor or build is running against it (any open UE 5.8 editor blocks builds repo-wide via the Live Coding mutex); UE recreates DDC/Intermediate on demand so the directories can go entirely. Afterwards run the full refresh (refresh-portfolio.ps1) and report the reclaimed GB from the Hygiene tiles, before/after. Close out with the Portfolio Signal (update this hand-off or prune it).`,
    ...
    {
      id: 'landscry-macro-03-bands',
      title: 'MACRO-03: macro-band measurement — and one credential step from you',
      target: 'Any agent · D:\Ascentia\repos\landscry',
      decision: false,
      why: 'MACRO-02 shipped 2026-08-04, mock-proven end to end through the running Studio: two prompts produced two genuinely different worlds on one seed, replay reproduced the identical review id, broken responses fail visibly with no partial artifact, and provenance names provider, model, prompt digest, and artifact hash. The frontier call itself is wired (official SDK, claude-opus-5, streaming, schema-locked) but has never run because this machine has no Anthropic credential - going live is one owner step, no code. MACRO-03 is the next slice: the macro diagnostic bands (512-2048 and 2048-8192 m) into the product scorecard, so the composed macro layer becomes measurable where the claim-bearing ladder is blind. Composer round 1 (2026-08-04, Claude as frontier composer per owner directive) built five authored archetypes through the seam and named four engine defects; those bundles under artifacts/frontier-macro-20260804 are ready test articles, and the bands make the amplitude-ceiling and quiet-zone defects numeric.',
      prompt: String.raw`Ship MACRO-03: macro-band measurement in the product scorecard. Also note the owner credential step below.
...
[NO USER DECISION REQUIRED for the slice itself. Do not reopen Unreal; do not touch the frozen reference fixture.]

Start in D:\Ascentia\repos\landscry on branch agent/s1-sediment (keep PR #2 open for CI). Read FIRST: docs\MACRO_COMPOSER_WORK_ORDER.md (MACRO-03 exit evidence), docs\MACRO_STRUCTURE_DIAGNOSIS_2026-08-03.md (the crossover finding and the band table this slice must reproduce), docs\MACRO02_COMPOSER_2026-08-04.md, docs\MACRO_COMPOSER_ROUND1_2026-08-04.md (four named defects; the five round-1 bundles are this slice's test articles), and the Landscry lane row in D:\Ascentia\ops\PIPELINE_STATUS.md.

THE SLICE: the claim-bearing band ladder (STANDARD_WAVELENGTH_BANDS_M, crates\landscry-terrain-eval\src\spectrum.rs) stops at 128-512 m - exactly the macro crossover, so the scorecard cannot see the macro deficit the diagnosis measured. Add DIAGNOSTIC macro bands (512-2048 and 2048-8192 m where the window extent allows) to the product dem_conformance scorecard as a separate clearly-labeled diagnostic block: not claim-bearing, no envelope comparison against the frozen fixture (which carries no macro-band envelopes and must not be re-frozen in this slice), published as gen-vs-window RMS ratios where a reference is computable and as absolute RMS otherwise. Use one band-RMS implementation shared with the existing spectral code - no duplicate math.

EXIT EVIDENCE: (1) the five family gallery pairs re-measured through the product path reproduce the diagnosis table ratios (alpine 0.81x at 2048-8192, karst 0.30x at 512-2048, etc.) within rounding; (2) a model-composed world bundle publishes its macro-band diagnostics - the five composer-round-1 bundles (artifacts/frontier-macro-20260804/builds, review ids in docs/MACRO_COMPOSER_ROUND1_2026-08-04.md) re-measured through the product path, quantifying the amplitude-ceiling and quiet-zone defects per band; (3) the block labeling states diagnostic-not-claim-bearing and names the band convention.

Laws: frozen reference fixture untouched; default byte-identity holds (measurement only, no terrain change); no gate promotion; honest claims.

Validation: cargo fmt --all -- --check; cargo check --locked --workspace --all-targets; cargo clippy --locked --workspace --all-targets -- -D warnings; cargo test --locked --workspace; corepack pnpm check; git diff --check. Close out with the Portfolio Signal per D:\Ascentia\repos\dashboard\PORTFOLIO_UPDATE_PROTOCOL.md, re-scope this hand-off to MACRO-04, and sweep artifacts per D:\Ascentia\ops\ARTIFACT_HYGIENE.md.`,
    },
...
In D:\Ascentia\repos\landscry (read AGENTS.md and the mountain-region lane row in D:\Ascentia\ops\PIPELINE_STATUS.md first): the validated 16-tile UE window package t4b-native-de1c19affdbe624b sits in artifacts\mountain-region-20260718 (225 MB, 80 artifacts, all checks green, same source identity as the review bundle). Run the import: one source round of 16 tiles + the WP MeshPartition builder with ByPackageHash reuse (verify-don't-resave definition assets) + optional HLOD pass as ONE BUILDER PROCESS PER HLOD ACTOR via -BuildSingleHLOD (labels carry the <HLODLayer>/ prefix) + inspect with the winding gate. Gotchas: editing anything under crates/ + Cargo + fixtures mid-build forks the engine cache identity; close editors before headless builds (Live Coding patch-DLL law; bypass flag is -NoHotReloadFromIDE); the terrain/water MATERIAL is still the open T4b presentation gate — this import proves geometry and streaming, not final look. Deliver a lit, flyable level plus its path for me to fly myself — renders are internal verification only. Close out with the Portfolio Signal.`,
  ...
  projects: [
    ...
    },
    {
      ...
      next: 'Two tracks: YOUR one-step credential (ANTHROPIC_API_KEY or ant auth login for the Studio server, then press Compose - the first live frontier composition is the remaining MACRO-02 acceptance run) and MACRO-03, the macro bands into the scorecard - composer round 1 already built its five test articles and named the defects the bands will quantify (amplitude ceiling, quiet-zone texture). Then engine slices in value order: amplitude fidelity, quiet-zone response, coast/ocean treatment for injected water; MACRO-04 families after. Conformance harness and CTRL-02 stay queued behind the macro lane.',
