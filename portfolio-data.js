...
In D:\Ascentia (read ops\ARTIFACT_HYGIENE.md first): delete ONLY the approved cache-class paths. Laws: repos\landscry\unreal\Binaries is generated-state, NOT cache — if I listed it, first prove a clean rebuild of the winding-fixed importer DLLs; never touch evidence/artifact/generated-state classes under a cache approval; before deleting a tree's caches confirm no editor or build is running against it (any open UE 5.8 editor blocks builds repo-wide via the Live Coding mutex); UE recreates DDC/Intermediate on demand so the directories can go entirely. Afterwards run the full refresh (refresh-portfolio.ps1) and report the reclaimed GB from the Hygiene tiles, before/after. Close out with the Portfolio Signal (update this hand-off or prune it).`,
    ...
    {
      id: 'landscry-macro-02-composer',
      title: 'MACRO-02: first frontier composition, end to end, behind explicit consent',
      target: 'Any agent · D:\Ascentia\repos\landscry',
      decision: false,
      why: 'MACRO-01 shipped 2026-08-04 (contract 0ea3bb7, seam 736f7a5, evidence docs/MACRO01_SEAM_2026-08-04.md): the macro layer is now a validated, replayable input, proven live with a hand-injected ring-range world that replays byte-identically. The next slice puts the frontier model behind that proven boundary: prompt to macro-design artifact to canonical validation to deterministic build, with consent recorded and no silent fallback. Everything risky about MACRO-02 is the network seam and the consent surface; the terrain side is already done.',
      prompt: String.raw`Ship MACRO-02: the first frontier-model macro composition, end to end. Mountain family only.

[NO USER DECISION REQUIRED for the architecture (ADR-0030 is accepted), but the API key/provider setup is the owner's: if no provider credential is configured, stop after wiring and testing the mock path and hand the credential step back. Never commit a credential. Do not reopen Unreal.]

Start in D:\Ascentia
epos\landscry on branch agent/s1-sediment (keep PR #2 open for CI). Read FIRST: docsdr0-model-composed-macro-design.md, docs\MACRO_COMPOSER_WORK_ORDER.md (MACRO-02 exit evidence), docs\MACRO01_SEAM_2026-08-04.md (the proven seam you are building on), and the Landscry lane row in D:\Ascentia\ops\PIPELINE_STATUS.md.

WHAT EXISTS: landscry.macro_design.v1 (validate() + canonical_sha256()); injected_macro_from_contract (public, engine) adding the v0.1 checks; V01ReferenceRequest::with_injected_macro_design binding the hash (custom-preview only); build_hydrology_v2_reference_with_macro threading the payload; manifest + report disclose macro_design {source, artifact_sha256}; the t4b_native_review CLI takes macro=<design.json>. The cloud_model provenance variant (provider/model/version/prompt_sha256/consent_grant_id) is schema'd and untested end to end.

THE SLICE: a Studio-side composer flow behind an explicit opt-in: user prompt -> frontier model call (provider adapter with a clean trait so the mock and the real provider share one path) -> the model returns the macro-design JSON -> canonical validation -> injected build -> bundle. The model prompt must instruct composition at design resolution with the amplitude bounds and the band-limit truth (the model shapes ranges and coasts; the engine owns detail). Record consent in the request per ADR-0030; a failed or invalid model response fails the request visibly with no partial bundle and NEVER falls back to the pinned preset silently.

EXIT EVIDENCE: (1) two different prompts produce visibly different macro layouts through the same seed (renders retained); (2) the same stored artifact replays byte-identically with the model unreachable (kill the network path and rebuild from the stored artifact); (3) a deliberately broken model response (truncated JSON, out-of-range weights) fails visibly with no partial bundle; (4) provenance in the bundle names provider, model, version, prompt digest, and artifact hash.

Laws: artifact-boundary law (nothing downstream of the artifact contacts a model); byte-identity of the default preset; no silent fallback; honest claims (the shipped description is "model-composed macro layout behind deterministic validation and replay" - no AAA claim, no "AI-native"); OFF-G gates stay deprioritized per ADR-0030.
...
epos\dashboard\PORTFOLIO_UPDATE_PROTOCOL.md, re-scope this hand-off to MACRO-03, and sweep artifacts per D:\Ascentia\ops\ARTIFACT_HYGIENE.md.`,
    },
...
In D:\Ascentia\repos\landscry (read AGENTS.md and the mountain-region lane row in D:\Ascentia\ops\PIPELINE_STATUS.md first): the validated 16-tile UE window package t4b-native-de1c19affdbe624b sits in artifacts\mountain-region-20260718 (225 MB, 80 artifacts, all checks green, same source identity as the review bundle). Run the import: one source round of 16 tiles + the WP MeshPartition builder with ByPackageHash reuse (verify-don't-resave definition assets) + optional HLOD pass as ONE BUILDER PROCESS PER HLOD ACTOR via -BuildSingleHLOD (labels carry the <HLODLayer>/ prefix) + inspect with the winding gate. Gotchas: editing anything under crates/ + Cargo + fixtures mid-build forks the engine cache identity; close editors before headless builds (Live Coding patch-DLL law; bypass flag is -NoHotReloadFromIDE); the terrain/water MATERIAL is still the open T4b presentation gate — this import proves geometry and streaming, not final look. Deliver a lit, flyable level plus its path for me to fly myself — renders are internal verification only. Close out with the Portfolio Signal.`,
  ...
  projects: [
    ...
    },
    {
      ...
      next: 'MACRO-02: the first frontier composition end to end — prompt to model to validated artifact to deterministic build, behind explicit consent, mountain family only. The provider credential step belongs to the owner. The conformance harness and CTRL-02 erosion time-depth remain queued inside the goal behind the macro lane.',
