...
In D:\Ascentia (read ops\ARTIFACT_HYGIENE.md first): delete ONLY the approved cache-class paths. Laws: repos\landscry\unreal\Binaries is generated-state, NOT cache — if I listed it, first prove a clean rebuild of the winding-fixed importer DLLs; never touch evidence/artifact/generated-state classes under a cache approval; before deleting a tree's caches confirm no editor or build is running against it (any open UE 5.8 editor blocks builds repo-wide via the Live Coding mutex); UE recreates DDC/Intermediate on demand so the directories can go entirely. Afterwards run the full refresh (refresh-portfolio.ps1) and report the reclaimed GB from the Hygiene tiles, before/after. Close out with the Portfolio Signal (update this hand-off or prune it).`,
    ...
    {
      id: 'landscry-ctrl02-erosion-depth',
      title: 'CTRL-02 erosion time-depth: the recorded lever on the final score — plus your one credential step',
      target: 'Any agent · D:\Ascentia\repos\landscry',
      decision: false,
      why: 'MACRO-03 shipped 2026-08-04: every review manifest now publishes Hann-windowed macro-band diagnostics (schema v12), the gallery reproduces the diagnosis table exactly through one shared implementation, and the composer amplitude ceiling is numeric (realized/composed 0.47-0.75 at 2-8 km). The acceptance validator pin (dead at v10 since the v11 fold) is healed - schema id single-sourced in contracts. The goal is the T3 ledger moving off 15 fail / 2 warn, and CTRL-02 erosion time-depth is the only mechanism with recorded evidence of moving the two metrics that fail in every family: alpine local_relief_p90 1.302 FAIL to 0.850 PASS at 100 intervals, fluvial 1.398 to 0.577 PASS (S1_SED05_CALIBRATION.md). The frontier call for live composition still waits only on a credential - one owner step, no code.',
      prompt: String.raw`Ship CTRL-02: the erosion time-depth control, in the ADR-0025/0026 pattern.
...
EXIT EVIDENCE: (1) default world byte-identical (frozen gates green); (2) the control moves local_relief_p90_m monotonically down across its ladder on at least alpine + fluvial family windows, with the measured table recorded; (3) request/manifest/acceptance carry the control with canonical validation and visible failure on out-of-range values; (4) no other registered control's declared metric moves beyond recorded tolerance (relief_scale independence measured).
...
In D:\Ascentia\repos\landscry (read AGENTS.md and the mountain-region lane row in D:\Ascentia\ops\PIPELINE_STATUS.md first): the validated 16-tile UE window package t4b-native-de1c19affdbe624b sits in artifacts\mountain-region-20260718 (225 MB, 80 artifacts, all checks green, same source identity as the review bundle). Run the import: one source round of 16 tiles + the WP MeshPartition builder with ByPackageHash reuse (verify-don't-resave definition assets) + optional HLOD pass as ONE BUILDER PROCESS PER HLOD ACTOR via -BuildSingleHLOD (labels carry the <HLODLayer>/ prefix) + inspect with the winding gate. Gotchas: editing anything under crates/ + Cargo + fixtures mid-build forks the engine cache identity; close editors before headless builds (Live Coding patch-DLL law; bypass flag is -NoHotReloadFromIDE); the terrain/water MATERIAL is still the open T4b presentation gate — this import proves geometry and streaming, not final look. Deliver a lit, flyable level plus its path for me to fly myself — renders are internal verification only. Close out with the Portfolio Signal.`,
  ...
  projects: [
    ...
    },
    {
      ...
      next: 'Two tracks: YOUR one-step credential (ANTHROPIC_API_KEY or ant auth login for the Studio server, then press Compose - the first live frontier composition is the remaining MACRO-02 acceptance run) and CTRL-02 erosion time-depth, the recorded lever on the two metrics failing in every family - the direct attack on the 15-fail/2-warn final score. The macro deficit and amplitude ceiling are now measured on every build (MACRO-03); amplitude fidelity and MACRO-04 families queue behind CTRL-02.',
