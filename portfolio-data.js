...
In D:\Ascentia (read ops\ARTIFACT_HYGIENE.md first): delete ONLY the approved cache-class paths. Laws: repos\landscry\unreal\Binaries is generated-state, NOT cache — if I listed it, first prove a clean rebuild of the winding-fixed importer DLLs; never touch evidence/artifact/generated-state classes under a cache approval; before deleting a tree's caches confirm no editor or build is running against it (any open UE 5.8 editor blocks builds repo-wide via the Live Coding mutex); UE recreates DDC/Intermediate on demand so the directories can go entirely. Afterwards run the full refresh (refresh-portfolio.ps1) and report the reclaimed GB from the Hygiene tiles, before/after. Close out with the Portfolio Signal (update this hand-off or prune it).`,
    ...
    {
      id: 'landscry-tex03-promotion',
      title: 'Your promotion decision on the TEX-02 ledger, then TEX-03 — plus your one credential step',
      target: 'Any agent · D:\Ascentia\repos\landscry',
      decision: true,
      why: 'TEX-02 shipped and measured 2026-08-05: physically parameterized texture conditioning in T1 (metre-true smoother + metre-coordinate fine-texture restore, never cell-scaled, byte-identical when off), per-family measured parameters, and the full 17-window candidate ledger HALVED the fail mass - 2/5/10 to 4 pass / 8 warn / 5 fail with zero regressions (run tex02-20260805 @ 3e60800; koc_center and koc_nw are the first karst passes in project history; all three aeolian windows rose to warn). Promotion of this configuration into the standing T3 candidate protocol is an owner decision on that evidence. The remaining five fails are named: badland x3 (the restore\u2019s finer octaves land in the unmeasured 2-8 m band; 32-128 residual 1.37x says badland wants more smoothing), koc_w (window-specific macro mismatch), gore_nw (one metric, slope_histogram 1.087).',
      prompt: String.raw`MY DECISION - promote the TEX-02 configuration as the standing T3 candidate configuration: [YES / NO / conditions]

Then ship TEX-03: close the badland texture gap.
...
THE SLICE: (1) add the 2-8 m band to the instrument (coverage-honest: only badland at 1 m cells resolves it fully); (2) re-measure bryce_west/east/amphitheater across all four bands under the shipped 20:33.5:16 config - the hypothesis on record is that the 33.5 m restore's finer fBm octaves (7.9/3.9 m) overfill 2-8 m and drive the surviving mean_slope/p90/concavity fails; (3) retune badland (restore octave count or length, larger sigma for the 1.37x residual at 32-128) against all four bands; (4) re-run the promoted-configuration ladder with the retuned badland map; no regression anywhere or halt. If the owner said YES above, also record the promotion (standing score moves to the new baseline; two-lever ledger doc pattern).
...
In D:\Ascentia\repos\landscry (read AGENTS.md and the mountain-region lane row in D:\Ascentia\ops\PIPELINE_STATUS.md first): the validated 16-tile UE window package t4b-native-de1c19affdbe624b sits in artifacts\mountain-region-20260718 (225 MB, 80 artifacts, all checks green, same source identity as the review bundle). Run the import: one source round of 16 tiles + the WP MeshPartition builder with ByPackageHash reuse (verify-don't-resave definition assets) + optional HLOD pass as ONE BUILDER PROCESS PER HLOD ACTOR via -BuildSingleHLOD (labels carry the <HLODLayer>/ prefix) + inspect with the winding gate. Gotchas: editing anything under crates/ + Cargo + fixtures mid-build forks the engine cache identity; close editors before headless builds (Live Coding patch-DLL law; bypass flag is -NoHotReloadFromIDE); the terrain/water MATERIAL is still the open T4b presentation gate — this import proves geometry and streaming, not final look. Deliver a lit, flyable level plus its path for me to fly myself — renders are internal verification only. Close out with the Portfolio Signal.`,
  ...
  projects: [
    ...
    },
    {
      ...
      next: 'YOUR DECISION IS READY: TEX-02 shipped and measured same-day as TEX-01 - the candidate ledger HALVES the fail mass, 2/5/10 -> 4 PASS / 8 WARN / 5 FAIL with zero regressions anywhere (run tex02-20260805). koc_center and koc_nw are the first karst passes in project history; all three aeolian windows rise fail -> warn. The mechanism is physically parameterized texture conditioning in T1 (a metre-true smoother plus a metre-coordinate fine-texture restore, never cell-scaled, byte-identical when off), per-family measured: karst 28:1.9:16, badland 20:33.5:16, aeolian 22:1.75:16. Promoting this as the standing T3 candidate configuration is your call on that evidence. Remaining fails are named: badland x3 (the 2-8 m band was unmeasured - TEX-03), koc_w (macro-side), gore_nw (one metric at 1.087). Fjordland r3 macro is composed at 257x257 with a fractal coast and 180 skerries; its engine build and ocean render are the next block. Owner gates still ready: OG-1 composer credential.',
      ...
      snapshot: { branch: 'agent/s1-sediment', head: 'aa5b659', date: '2026-08-05', dirty: 0, ahead: 0, behind: 0 },
    },
    {
      id: 'worldheart',
      name: 'Worldheart',
