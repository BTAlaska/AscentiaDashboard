...
In D:\Ascentia (read ops\ARTIFACT_HYGIENE.md first): delete ONLY the approved cache-class paths. Laws: repos\landscry\unreal\Binaries is generated-state, NOT cache — if I listed it, first prove a clean rebuild of the winding-fixed importer DLLs; never touch evidence/artifact/generated-state classes under a cache approval; before deleting a tree's caches confirm no editor or build is running against it (any open UE 5.8 editor blocks builds repo-wide via the Live Coding mutex); UE recreates DDC/Intermediate on demand so the directories can go entirely. Afterwards run the full refresh (refresh-portfolio.ps1) and report the reclaimed GB from the Hygiene tiles, before/after. Close out with the Portfolio Signal (update this hand-off or prune it).`,
    ...
    {
      id: 'landscry-gpu-r8',
      title: 'Macro-brilliance program: r9 genotype composer from the atlas grammars, then selection rounds',
      target: 'Any agent · D:\\Ascentia\\repos\\landscry',
      decision: false,
      why: String.raw`GPU-04/05 + LSC-03 (2026-08-07, 7192d8d..0773545 on agent/s1-sediment, pushed) proved 16k end-to-end: 16,384^2 @ 1 m fjordland in 160 s (~80x CPU), SIA glacial pass (emergent U-troughs, flotation sills), sea-connected flood, DEM score 11 pass / 0 warn / 1 fail vs alpine_glaciated, and a ready UE package (artifacts\gpu-erode\landscape-fjord-r7c-16k, WP import steps in its README). Three opens: the composed skeleton's straight distance-field ridges are now the visible quality ceiling; the residual LocalReliefP90 fail cannot be interpreted without a fjord-specific instrument (no Norwegian-fjord DEM window exists); and SED-08 (continue_sediment_flux) still sits UNCOMMITTED in crates\landscry-terrain-core\src\sediment.rs.`,
      prompt: String.raw`GPU fjord lane round 2, in D:\Ascentia\repos\landscry on agent/s1-sediment. Read FIRST: docs\GPU04_FJORD_16K_2026-08-07.md (all laws: scalar rasters, B-spline-not-Catmull-Rom, massif-only hardness at glacial apply, fenced chunks, cell-size-dependent hydraulics), the r7 composer artifacts\frontier-macro-20260807\compose_fjordland_r7_gpu.py, and crates\landscry-gpu\examples\gpu_erode.rs.

READ FIRST ALSO: docs\FJORD_GENOTYPE_ATLAS_2026-08-07.md, docs\MACRO_HERO_CONSTRAINTS_2026-08-07.md, docs\MACRO_STORY_EVOLUTION_2026-08-07.md (the owner's macro-brilliance program, 2026-08-07 PM: #1-3 before the #4 selection loop; ITERATION LAW: fresh seed every round). THE HEADLINE BITE: r9 genotype composer - sample the atlas grammars (scale class, trunk segments+doglegs, arm fan, width laws, sill/bathymetry, districts) into a genotype JSON, assemble the signed macro from it, evaluate hero predicates, and run a 16-candidate contact-sheet selection round at 1k preview. THEN, EACH GREEN-GATED: (1) SED-08 first - the working tree carries continue_sediment_flux in sediment.rs, proven per the 2026-08-06 board note but never committed; run the full validation set and commit it alone. (2) Fjordland r8: apply the r3 coast treatment INLAND - noise-displace every ridge/valley polyline claim (fBm perturbation of the distance field or of the polyline points) so straight distance-field ridge lines disappear; keep the identical skeleton constants, seed, and signed pilot-bathymetry contract; rebuild at 1025, run the r7c-winning params (dt=0.03 capacity=1.5 dissolve=0.35 max_cut=0.35 thermal=0.3 detail_amp=25 gsteps=512 polish=600 ela=1250), and A/B the overview + arm crop against artifacts\gpu-erode\fjord_r7c_16k.png. (3) Glacial instrument: a trough cross-section power-law fit (b in h = a*x^b across valley transects; glacial b ~= 1.5-2.5, fluvial ~= 1) as a new example or eval_surface extension, measured on r7c vs the pre-glacial surface, so the standing LocalReliefP90 fail gets an interpretable companion number instead of an alpine-corpus verdict.

[USER STEP, SEPARATE: import artifacts\gpu-erode\landscape-fjord-r7c-16k into a UE 5.8.1 World Partition map per its README and fly it - this is the acceptance that matters; the GPU lane stays PROVISIONAL until your eye rules.]

Laws: presentation model - no product-path or gate claims; never touch landscry-terrain-core defaults; artifacts stay in artifacts\gpu-erode + artifacts\frontier-macro-20260807; explicit-path staging only (SED-08 tree). Validation: cargo fmt --all -- --check; cargo clippy --release -p landscry-gpu -p landscry-terrain-calibration --all-targets; cargo test -p landscry-terrain-core; python renders via artifacts\gpu-erode\render_hillshade.py. Close out with the Portfolio Signal per D:\Ascentia\repos\dashboard\PORTFOLIO_UPDATE_PROTOCOL.md, re-scope this hand-off, and sweep per D:\Ascentia\ops\ARTIFACT_HYGIENE.md.`,
    },
    {
      id: 'landscry-tex03',
...
In D:\Ascentia\repos\landscry (read AGENTS.md and the mountain-region lane row in D:\Ascentia\ops\PIPELINE_STATUS.md first): the validated 16-tile UE window package t4b-native-de1c19affdbe624b sits in artifacts\mountain-region-20260718 (225 MB, 80 artifacts, all checks green, same source identity as the review bundle). Run the import: one source round of 16 tiles + the WP MeshPartition builder with ByPackageHash reuse (verify-don't-resave definition assets) + optional HLOD pass as ONE BUILDER PROCESS PER HLOD ACTOR via -BuildSingleHLOD (labels carry the <HLODLayer>/ prefix) + inspect with the winding gate. Gotchas: editing anything under crates/ + Cargo + fixtures mid-build forks the engine cache identity; close editors before headless builds (Live Coding patch-DLL law; bypass flag is -NoHotReloadFromIDE); the terrain/water MATERIAL is still the open T4b presentation gate — this import proves geometry and streaming, not final look. Deliver a lit, flyable level plus its path for me to fly myself — renders are internal verification only. Close out with the Portfolio Signal.`,
  ...
  projects: [
    ...
    },
    {
      ...
      state: 'active',
      phase: 'Production goal active (steward) — CPU standing score 4/8/5; GPU 16k lane proven 2026-08-07 (provisional, owner evaluation open)',
