...
In D:\Ascentia (read ops\ARTIFACT_HYGIENE.md first): delete ONLY the approved cache-class paths. Laws: repos\landscry\unreal\Binaries is generated-state, NOT cache — if I listed it, first prove a clean rebuild of the winding-fixed importer DLLs; never touch evidence/artifact/generated-state classes under a cache approval; before deleting a tree's caches confirm no editor or build is running against it (any open UE 5.8 editor blocks builds repo-wide via the Live Coding mutex); UE recreates DDC/Intermediate on demand so the directories can go entirely. Afterwards run the full refresh (refresh-portfolio.ps1) and report the reclaimed GB from the Hygiene tiles, before/after. Close out with the Portfolio Signal (update this hand-off or prune it).`,
    ...
    {
      id: 'landscry-macro-01-seam',
      title: 'MACRO-01: open the macro-design seam, model-free, byte-identical default',
      target: 'Any agent · D:\\Ascentia\\repos\\landscry',
      decision: false,
      why: 'Owner direction 2026-08-03 (ADR-0030): a frontier model composes the macro layer from a user prompt; the deterministic engine builds it; full-offline is deprioritized for now. MACRO-01 is the only slice that touches accepted composition code, so it goes first and alone: define the artifact contract and open the seam with the pinned realization as the byte-identical default producer. Everything after it — the first live model call, the macro measurement bands, families under the envelope — stacks on this seam.',
      prompt: String.raw`Ship MACRO-01: the landscry.macro_design.v1 contract and the composition seam. Model-free.

[NO USER DECISION REQUIRED. Owner direction is recorded in docs\adr\0030-model-composed-macro-design.md. Do not call any model or add any network path in this slice. Do not reopen Unreal.]

Start in D:\Ascentia\repos\landscry on branch agent/s1-sediment (keep PR #2 open — CI runs only on main and pull requests). Read FIRST: docs\adr\0030-model-composed-macro-design.md, docs\MACRO_COMPOSER_WORK_ORDER.md (MACRO-01 exit evidence), docs\MACRO_STRUCTURE_DIAGNOSIS_2026-08-03.md, .agent\GOAL.md finding 0b, and the Landscry lane row in D:\Ascentia\ops\PIPELINE_STATUS.md.

THE CONTRACT: landscry.macro_design.v1 carries exactly what compose_v01_northern_range_pre_hydrology consumes — the six design-resolution rasters (macro_height_m f32, crest_weight, flank_weight, coast_damp, slope_weight in [0,1], land_deep mask) plus world extent, family id, declared amplitude bounds, and source provenance (source class, provider/model/version, prompt_sha256, consent grant — all optional-but-schema'd now, exercised by MACRO-02). Versioned schema + Rust type + generated JSON Schema + TS SDK + cross-language fixtures, through the same generator flow as every other contract (crates\landscry-contracts, pnpm generate:contracts / generate:contract-fixtures). Canonical Rust validation: finite values, exact dimensions, declared ranges, land-fraction sanity, schema identity. Malformed artifacts fail visibly.

THE SEAM: the internal MacroDesign struct already exists (composition\v01_baseline\mod.rs:1249). Make build_northern_range_macro the named default PRESET PRODUCER emitting the same artifact type, and let the composition path accept an injected validated artifact instead. The artifact SHA-256 binds into the composition cache identity exactly as CTRL-01's relief value does (v01_reference_world.rs apply_relief_scale is the identity-binding pattern to follow). Injected artifacts are custom-preview authority only, like every non-default control.

EXIT EVIDENCE (all three, recorded in .agent\ITERATION_LOG.md and a docs\MACRO01_*.md evidence doc): (1) the default path is BYTE-IDENTICAL — reference world source-height and terrain-bed SHA-256 unchanged from the SEED-01 reference artifact, frozen characterization gates green; (2) a hand-made test artifact (e.g., shifted range axis) produces a different, deterministic, replayable world whose identity re-keys from the artifact hash — same artifact twice = same bundle identity; (3) validation rejects wrong dimensions, non-finite values, out-of-range weights, and unknown schema versions with visible errors and no partial output.

Laws: mechanism-per-axis does not apply here (this is a composition input, not a control) but byte-identity and no-silent-fallback do; promote no gate; the T4b materials lane and Unreal stay shelved; claims end at "model-composed macro layout behind deterministic validation and replay" — no AAA claim, no "AI-native".

Validation: cargo fmt --all -- --check; cargo check --locked --workspace --all-targets; cargo clippy --locked --workspace --all-targets -- -D warnings; cargo test --locked --workspace; corepack pnpm check; git diff --check. Close out with the Portfolio Signal per D:\Ascentia\repos\dashboard\PORTFOLIO_UPDATE_PROTOCOL.md, re-scope this hand-off to MACRO-02, and sweep artifacts per D:\Ascentia\ops\ARTIFACT_HYGIENE.md.`,
    },
...
In D:\Ascentia\repos\landscry (read AGENTS.md and the mountain-region lane row in D:\Ascentia\ops\PIPELINE_STATUS.md first): the validated 16-tile UE window package t4b-native-de1c19affdbe624b sits in artifacts\mountain-region-20260718 (225 MB, 80 artifacts, all checks green, same source identity as the review bundle). Run the import: one source round of 16 tiles + the WP MeshPartition builder with ByPackageHash reuse (verify-don't-resave definition assets) + optional HLOD pass as ONE BUILDER PROCESS PER HLOD ACTOR via -BuildSingleHLOD (labels carry the <HLODLayer>/ prefix) + inspect with the winding gate. Gotchas: editing anything under crates/ + Cargo + fixtures mid-build forks the engine cache identity; close editors before headless builds (Live Coding patch-DLL law; bypass flag is -NoHotReloadFromIDE); the terrain/water MATERIAL is still the open T4b presentation gate — this import proves geometry and streaming, not final look. Deliver a lit, flyable level plus its path for me to fly myself — renders are internal verification only. Close out with the Portfolio Signal.`,
  ...
  projects: [
    ...
    },
    {
      ...
      next: 'MACRO-01: define landscry.macro_design.v1 and open the composition seam, model-free, with the pinned northern-range realization as the byte-identical default preset producer. Then MACRO-02, the first frontier composition end to end behind explicit consent. The conformance harness and CTRL-02 erosion time-depth remain queued inside the goal behind the macro lane.',
