...
| Game World Asset Factory | Future P17/P23 non-structural world-asset quarantine intake, `EUW_AscentiaLODAndCollisionDoctor`, PCG provenance tags, Worldheart attachment metadata, and world-zone review | Generated static meshes remain external source material. Owner law assigns buildings and structural meshes to Worldheart; Meshy authors one atomic terrain/PCG mesh, prop, or reviewed decorative attachment while PCG/manual placement owns piles, fields, clusters, and dressing. The locked v4 catalog contains 61 archetypes and 1,161 review-compatible plans: 891 Meshy 6 Nanite-active, 90 T2-paused, and 180 historical review-only. Physical has 55 active Meshy 6 entries plus 2 paused T2 entries, including 36 new atomic geology forms. T2 is hard-blocked at prompt-Go, queue, reference-generation, and provider layers. The implemented reference path generates one Images 2 image and sends it to Meshy's single-image endpoint; no multi-view path exists. Of 79 completed packages, 50 remain current non-structural candidates at 6 Keep / 0 Reject / 44 awaiting review; 29 structural results are historical quarantine only. | No generated mesh may bypass `ImportQueue/WorldAssets`, source/task provenance, Blender QA, a separate result Keep, Unreal quarantine, and a human Nanite/PCG/Worldheart-mount verdict. Keep retains a candidate only. No old approval is valid; no job resumes without a new exact Meshy 6-only v4 user decision, hash, ID list, and cap. T2, review-only composites, and structural results cannot enter the current generation round. |
...
3. Game World Asset Factory: continue separate hash-bound Keep or Reject review
   for the 50 landed packages (6 Keep / 44 awaiting), keep 29 structural
   packages as historical quarantine only, review the 36-form Physical atomic
   geology prompt library, decide whether an identity-consistent multi-view
   reference experiment is required, and require a new explicit Meshy 6-only v4
   decision before any small pilot. T2 is paused, Prompt Go never accepts a
   provider result, PCG owns rubble and debris composition, and Keep still
   requires later Unreal Nanite/PCG and Worldheart-mount verdicts.
