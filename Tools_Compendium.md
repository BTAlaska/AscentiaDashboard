...
| Game World Asset Factory | Future P17/P23 world-asset quarantine intake, `EUW_AscentiaLODAndCollisionDoctor`, PCG provenance tags, and world-zone review | Generated static meshes remain external source material. The human owner approved a hash-bound 120-asset Physical/Magical/Chilled batch capped at 2,295 Meshy credits. The first asset proves the GPT Image 2 → Meshy → Blender → `ImportQueue/WorldAssets` evidence route, but its Blender warning (3,491 non-manifold/boundary edges; 194 islands) and all missing Unreal checks remain explicit debt. | No generated mesh may bypass `ImportQueue/WorldAssets`, source/task provenance, Blender QA, Unreal quarantine, and a human Nanite/PCG verdict. The active approval cannot expand to another theme, exceed 2,295 credits, authorize paid retries, or count provider completion as production acceptance. |
...
3. Game World Asset Factory: finish the explicitly approved 120-asset,
   2,295-credit Physical/Magical/Chilled batch without scope expansion or paid
   retries, verify every `ImportQueue/WorldAssets` package, then keep all results
   in external quarantine until human Blender and Unreal Nanite/PCG verdicts
   exist.
